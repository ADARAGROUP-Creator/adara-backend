...
### Pricing (nuevo dominio, 25/9/2026)

| # | Regla | Doc |
|---|-------|-----|
| **PRC1** | **Pricing es una herramienta de SIMULACIÓN, no de administración (decisión de Sebastián, 25/9/2026).** Sirve para simular rápido la rentabilidad de un producto y fijar precios. Usa **su propio costo editable** (`cost_without_vat`, costo de reposición) y **sus propias tasas simples** (p. ej. IIBB 5 %). **No tiene que coincidir** con el costo FIFO (CF6) ni con la alícuota efectiva del CM03 (IIBB3): la diferencia es chica (5 % vs 5,29 %) y no afecta nada fiscal. | `ADARA-PRICING.md` |
| **PRC2** | **Los números de pricing nunca alimentan la administración.** Resultado, CMV, stock, posición fiscal e IIBB siguen saliendo **sólo** de FIFO, `ventas_ml` y el CM03. Al migrar pricing a ADARA APP, su costo y sus tasas van a **tablas propias de pricing**: no se escriben en `skus.costo_referencia`, `lotes` ni `iibb_parametros`. Corolario: pricing **puede mostrar** el costo FIFO como referencia, pero no lo reemplaza ni lo modifica. | `ADARA-PRICING.md` |
