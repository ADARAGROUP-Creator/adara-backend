...
**Plan por pasos** (de menor a mayor riesgo): 1. catálogo (match `products.sku` → `skus.id`, EAN, categoría, medidas) · 2a. motor de precios ✅ · 2b. tablas propias ✅ (25/9, ver `ADARA-SCHEMA.md`) + carga de datos y, tasas, cuotas, comisiones y márgenes + pantalla de Precios sólo lectura · 3. escrituras en ML (precio, B2B, promociones, preguntas), recién con la conexión de ML unificada · 4. análisis ML sobre `ventas_ml` · 5. logística y Tienda Nube.
