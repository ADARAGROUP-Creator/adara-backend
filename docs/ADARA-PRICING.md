...
**Plan por pasos** (de menor a mayor riesgo): 1. catálogo (match `products.sku` → `skus.id`, EAN, categoría, medidas) · 2a. motor de precios ✅ · 2b. tablas propias ✅ (25/9, ver `ADARA-SCHEMA.md`) + pantalla **Precios** ✅ (25/9, `#precios`) + carga de datos y, tasas, cuotas, comisiones y márgenes + pantalla de Precios sólo lectura · 3. escrituras en ML (precio, B2B, promociones, preguntas), recién con la conexión de ML unificada · 4. análisis ML sobre `ventas_ml` · 5. logística y Tienda Nube.

**25/9/2026 — Pantalla Precios (`#precios`).** Grilla de SKUs activos × canales activos con precio redondeado (con IVA) y margen neto sobre venta. Edición inline del costo de simulación, IVA (21 / 10,5) y categoría ML; modal de detalle por SKU con envío y cargo fijo de ML (brutos, manuales hasta el sync de publicaciones), margen %, utilidad neta y PVP manual por canal (prioridad PVP > utilidad > margen, igual que pricing) y desglose en vivo. Bloque de configuración con tasas, canales (costo de cuotas, redondeo, activo) y comisión por categoría.
- Fidelidad con pricing: sin margen cargado usa **5 %** (como `getMargin` de pricing); `margen_default_pct` de `pricing_canal` queda sin uso, igual que `default_margin_rate` en pricing. Redondeo por canal (default $100 al más cercano, el de pricing).
- Verificado en vista previa con base simulada (sin login, sin tocar la base real): SKU 101 en MC = **$500.800** (6,8 %); PVP manual MP3 $550.000 → comisión $95.000, IIBB $24.887, ganancia $29.826, cuentas a mano OK; guardar persiste sólo el canal cambiado; IIBB 5,26 % recalcula MC a $502.300; sin desborde en 375 px, 966 px y 1440 px.
- Pendiente: cargar los 132 costos, categorías, comisiones y márgenes reales desde la base de pricing (pregunta 10 a Codex).
